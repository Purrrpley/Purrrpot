...
def command(
  ...
  if args.amount > 1:
    with ThreadPoolExecutor() as e:
      # `e.map()` requires that you pass an argument to the function, so we 
      # workaround that by giving it a lambda function that takes an (unused) 
      # argument and executes the right function without any arguments.
